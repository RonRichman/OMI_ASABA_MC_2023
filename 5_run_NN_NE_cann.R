...
output = list(final,input_list[[count + 1]])  %>% layer_multiply(name = "output")
...
model_write = callback_model_checkpoint(paste0("c:/r/asaba_frmtpl_nn_ne_cann.h5"), save_best_only = T, verbose = 1)
...
model = load_model_hdf5("c:/r/asaba_frmtpl_nn_ne_cann.h5")
