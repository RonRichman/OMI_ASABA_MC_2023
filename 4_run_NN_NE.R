...
output = middle %>% 
  list(middle,input_list[[count + 1]])  %>% layer_multiply(name = "output")
...
model_write = callback_model_checkpoint(paste0("c:/r/asaba_frmtpl_nn_ne.h5"), save_best_only = T, verbose = 1)
...
model = load_model_hdf5("c:/r/asaba_frmtpl_nn_ne.h5")
...
NE_model = keras_model(model$inputs, model$layers[[25]]$output)
age_embedding = NE_model %>% predict(test_x, batch_size = 32000) %>% data.table()
age_embedding[, DrivAge := test$DrivAge]

age_embedding %>% unique %>%  melt.data.table(id.vars = "DrivAge") %>% 
  ggplot(aes(x = DrivAge, y= value))+geom_point(aes(colour = variable))

NE_model = keras_model(model$inputs, model$layers[[26]]$output)
age_embedding = NE_model %>% predict(test_x, batch_size = 32000) %>% data.table()
age_embedding[, BonusMalus := test$BonusMalus]

age_embedding %>% unique %>%  melt.data.table(id.vars = "BonusMalus") %>% 
  ggplot(aes(x = BonusMalus, y= value))+geom_point(aes(colour = variable))
